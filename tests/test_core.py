...
def test_get_supported_domains() -> None:
    ...
    assert isinstance(domains, list)  # noqa: S101
    assert all(isinstance(domain, str) for domain in domains)  # noqa: S101
...
def test_is_supported() -> None:
    ...
    assert isinstance(TrueLinkResolver.is_supported("https://www.google.com"), bool)  # noqa: S101
    assert not TrueLinkResolver.is_supported("https://www.google.com")  # noqa: S101
    ...
    if supported_domains:
        ...
        assert TrueLinkResolver.is_supported(supported_url)  # noqa: S101
...
async def test_caching() -> None:
    ...
    assert result1 is result2  # noqa: S101
