...
class MediaFireResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        if "/folder/" in base_url_for_checks:  # Use base_url_for_checks for routing
            return await self._resolve_folder(
                url, _password
            )  # Pass original url for processing
    ...
    async def _resolve_file(
        ...
        if re.search(r"https?:\/\/download\d+\.mediafire\.com\/\S+\/\S+\/\S+", url):
            ...
            try:
                ...
                Exception
            ):  # Fallback if _fetch_file_details fails or not implemented
        ...
        try:
            ...
            Exception
        ) as e:  # Catch other exceptions like requests.exceptions.RequestException
    ...
    async def _api_request(
        ...
        if (
            api_res.get("result", "").lower() == "error" or "message" in api_res
    ...
    async def _resolve_folder(self, url: str, password: str) -> FolderResult:
        ...
        scraper.headers.update(
            {"User-Agent": BaseResolver.USER_AGENT}
        )  # Set User-Agent
        ...
        try:
            ...
            try:
                ...
                if (
                    not folder_keys_list or not folder_keys_list[0]
                ):  # Ensure there's at least one valid key
            ...
            if (
                "folder_infos" in folder_info_response
            ):  # Multiple folders' info returned
            ...
            async def get_folder_contents_recursive(
                ...
                for file_api_data in api_files:
                    ...
                    try:
                        ...
                        if (
                            item_size is None and "size" in file_api_data
                        ):  # Fallback to API size
                        ...
                        file_item = FileItem(
                            ...
                            size=item_size,
                            path=ospath.join(
                                current_path_prefix, item_filename
                            ),  # Path should be relative to folder root
                        )
