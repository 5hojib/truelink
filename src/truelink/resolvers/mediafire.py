...
class MediaFireResolver(BaseResolver):
    ...
    async def _repair_download(self, repair_url: str, original_password: str) -> LinkResult | FolderResult:
    ...
    async def _resolve_file(self, url: str, password: str) -> LinkResult:
        ...
        normalized_url = f"{parsed_url_obj.scheme}://{parsed_url_obj.netloc}{parsed_url_obj.path}"
        ...
        try:
            ...
            if error_elements:
                raise ExtractionFailedException(f"MediaFire error: {error_elements[0]}")

            if html.xpath("//div[@class='passwordPrompt']"):
                if not password:
                    raise ExtractionFailedException(PASSWORD_ERROR_MESSAGE.format(normalized_url))

                async with await self._post(normalized_url, data={"downloadp": password}) as pw_response:
                    html_text = await pw_response.text()
                html = fromstring(html_text)

                if html.xpath("//div[@class='passwordPrompt']"): # Password was wrong
                    raise ExtractionFailedException("MediaFire error: Wrong password.")

            final_link_elements = html.xpath('//a[@aria-label="Download file"]/@href')
            if not final_link_elements:
                ...
                if repair_link_elements:
                    ...
                    return await self._repair_download(repair_link_elements[0], password)
                raise ExtractionFailedException("No download or repair link found on MediaFire page.")
            ...
            if "mediafire.com" in urlparse(final_link).hostname and not re.match(r"https?:\/\/download\d+\.mediafire\.com", final_link):
            ...
            if isinstance(e, (ExtractionFailedException, InvalidURLException)):
                raise
            raise ExtractionFailedException(f"Failed to resolve MediaFire file '{url}': {e!s}") from e

    async def _fetch_folder_content_page(self, folder_key: str, content_type: str = "files"):
        ...
        async with await self._get(api_url, params=params) as response:
            if response.status != 200:
                raise ExtractionFailedException(f"Failed to get folder content API ({content_type}): {response.status}")
            json_data = await response.json()

        res_api = json_data.get("response", {})
        if "message" in res_api and res_api.get("result", "").lower() == "error":
            raise ExtractionFailedException(f"MediaFire API error for folder content ({content_type}): {res_api['message']}")
    ...
    async def _resolve_folder(self, url: str, password: str) -> FolderResult:
        ...
        try:
            ...
            if not folder_keys_list:
                raise InvalidURLException("Could not parse folder key from MediaFire URL.")
            ...
            async with await self._post(
                ...
                data={
                    ...
                    "folder_key": folder_keys_list[0], # Using the first key for main info
                    "response_format": "json",
                },
            ) as response:
                if response.status != 200:
                    raise ExtractionFailedException(f"Failed to get folder info: {response.status}")
                json_data = await response.json()

            api_response = json_data.get("response", {})
            if api_response.get("result", "").lower() == "error" or "folder_info" not in api_response:
                 # Try folder_infos for multiple keys case, though we only used one key above
                if "folder_infos" in api_response and api_response["folder_infos"]:
                    folder_title = api_response["folder_infos"][0].get("name", "MediaFire Folder")
                elif "message" in api_response:
                     raise ExtractionFailedException(f"MediaFire API error for folder info: {api_response['message']}")
                else:
                    raise ExtractionFailedException("MediaFire API error: Could not retrieve folder_info.")
            else:
                folder_title = api_response["folder_info"].get("name", "MediaFire Folder")

            all_files: list[FileItem] = []
            total_size = 0

            async def process_folder_key_contents(current_folder_key: str, current_path: str):
                nonlocal total_size # To modify total_size from outer scope
                # Get files in the current folder
                files_content = await self._fetch_folder_content_page(current_folder_key, "files")
                for file_info in files_content.get("files", []):
                    ...
                    try:
                        link_result = await self._resolve_file(file_url_page, password)
                        file_item = FileItem(
                ...
                subfolders_content = await self._fetch_folder_content_page(current_folder_key, "folders")
            ...
            for key in folder_keys_list:
                 ...
                 path_prefix = ""
                 if len(folder_keys_list) == 1: # If only one key, use the main folder title as base path
                     path_prefix = folder_title
                 ...
                 current_folder_info_resp = await self._post(
                     folder_api_url, data={"folder_key": key, "response_format": "json"})
                 current_folder_info_json = await current_folder_info_resp.json()
                 current_folder_name_for_path = current_folder_info_json.get("response",{}).get("folder_info",{}).get("name", key)

                 await process_folder_key_contents(key, current_folder_name_for_path if len(folder_keys_list) > 1 else folder_title)


            if not all_files:
                raise ExtractionFailedException(f"No downloadable files found in MediaFire folder '{url}'.")
            ...
            if isinstance(e, (ExtractionFailedException, InvalidURLException)):
                raise
            raise ExtractionFailedException(f"Failed to resolve MediaFire folder '{url}': {e!s}") from e
