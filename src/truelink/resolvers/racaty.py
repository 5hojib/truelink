...
class RacatyResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not file_id:
                raise InvalidURLException(f"Could not extract file ID from Racaty URL: {canonical_url}")

            post_data = {"op": "download2", "id": file_id}

            # POST to the canonical URL
            async with await self._post(canonical_url, data=post_data) as post_response:
            ...
            if not direct_link_elements:
                ...
                if error_msg:
                    raise ExtractionFailedException(f"Racaty error: {error_msg[0].strip()}")

                # Check for common messages like "File not found"
                if "File Not Found" in response_text or "No such file" in response_text:
                     raise ExtractionFailedException(f"Racaty error: File Not Found or link expired for ID {file_id}.")

                raise ExtractionFailedException("Racaty error: Direct download link ('uniqueExpirylink') not found.")
            ...
            filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": canonical_url})
