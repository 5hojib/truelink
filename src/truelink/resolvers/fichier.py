...
PASSWORD_ERROR_MESSAGE_FICHIER = "1Fichier link {} requires a password (append ::password to the URL)."
...
class FichierResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        if not re.match(regex_1fichier, url.split("::")[0]): # Check URL part before password
        ...
        try:
            ...
            dl_url_elements = html.xpath('//a[@class="ok btn-general btn-orange"]/@href')
            if dl_url_elements:
                direct_link = dl_url_elements[0]
                filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": request_url})
            ...
            if not ct_warn_elements:
                ...
                if "In order to access this file, you will have to validate a first download." in response_text:
                     raise ExtractionFailedException("1Fichier error: Requires a prior validation download (often via browser). Link may be restricted.")
                raise ExtractionFailedException(
            ...
            if len(ct_warn_elements) >= 1:
                ...
                last_warn_text_content = "".join(ct_warn_elements[-1].xpath(".//text()")).lower().strip()

                if "you must wait" in last_warn_text_content:
                    numbers = [int(s) for s in last_warn_text_content.split() if s.isdigit()]
                    wait_time_msg = f"Please wait {numbers[0]} minute(s)." if numbers else "Please wait a few minutes/hours."
                    raise ExtractionFailedException(f"1Fichier error: Download limit reached. {wait_time_msg}")

                if "bad password" in last_warn_text_content:
                    raise ExtractionFailedException("1Fichier error: The password you entered is wrong.")

                if "you have to create a premium account" in last_warn_text_content:
                    raise ExtractionFailedException("1Fichier error: This link may require a premium account.")

                # Check for password prompt if password was not provided
                if "protect access to this file" in last_warn_text_content or "enter the password" in last_warn_text_content:
                    if not _password:
                        raise ExtractionFailedException(PASSWORD_ERROR_MESSAGE_FICHIER.format(request_url))
            ...
            all_warnings = " | ".join(["".join(w.xpath(".//text()")).strip() for w in ct_warn_elements])
            raise ExtractionFailedException(
