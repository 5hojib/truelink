...
class PCloudResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            script_json_match = re.search(r"<script[^>]*>\s*(?:var\s+|window\.)?\w+\s*=\s*(\{.*?\});?\s*</script>", response_text, re.DOTALL | re.IGNORECASE)
            if script_json_match:
                try:
                    ...
                    if 'downloadlink' in metadata and isinstance(metadata['downloadlink'], str):
                        direct_link = metadata['downloadlink']
                    elif 'downloadLink' in metadata and isinstance(metadata['downloadLink'], str): # Camel case
                        direct_link = metadata['downloadLink']
            ...
            if not direct_link:
                ...
                cdn_links = re.findall(r'["\'](https://[a-zA-Z0-9.-]+\.pcloud.com/[^"\']+)["\']', response_text)
                if cdn_links:
                    ...
                    for cdn_link in cdn_links:
                        if any(ext in cdn_link.lower() for ext in ['.zip', '.rar', '.exe', '.iso', '.mp4', '.mkv']) or 'download=1' in cdn_link:
                            direct_link = cdn_link
                            break
                    if not direct_link and cdn_links: # If no specific pattern, take the first one
                        direct_link = cdn_links[0]


            if not direct_link:
                raise ExtractionFailedException("pCloud.link error: Direct download link not found in page source.")
            ...
            if not filename and direct_link:
                try:
                    ...
                    if path_part and path_part != "/":
                        ...
                        if '.' in potential_filename and not potential_filename.split('.')[0].isdigit():
