...
class GitHubResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not (parsed_url.hostname == "github.com" and "/releases/" in parsed_url.path):
                ...
                if not re.search(r"github\.com.*/releases/", url):
                     raise InvalidURLException(f"URL '{url}' does not appear to be a GitHub Releases link.")
            ...
            try:
                async with await self._get(url, allow_redirects=True) as response: # Try HEAD first
                    ...
                    if content_disposition:
                        match_fn = re.search(r"filename=[\"']?([^\"']+)[\"']?", content_disposition)
                        if match_fn:
                            temp_filename = match_fn.group(1)

                    if not temp_filename: # Fallback to path if no content-disposition
                ...
                async with await self._get(url, allow_redirects=False) as response:
                    if response.status in (301, 302, 303, 307, 308) and "location" in response.headers:
                        ...
                        if response.status == 200:
                             final_url_after_redirects = url # Or raise, as this is unusual for /download/ paths
                        else:
            ...
            if not final_url_after_redirects:
                 raise ExtractionFailedException(f"GitHub: Unable to extract the final asset link from '{url}'.")
            ...
            if temp_filename is None or temp_size is None:
                filename, size = await self._fetch_file_details(final_url_after_redirects)
            else:
            ...
            return LinkResult(url=final_url_after_redirects, filename=filename, size=size)
