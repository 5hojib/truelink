    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not path_segments:
                raise InvalidURLException("Streamvid: Invalid URL, no path segments found.")

            file_code_with_quality = path_segments[-1] # e.g., filecode_o or just filecode
            ...
            quality_suffixes = ["_o", "_h", "_n", "_l"] #_o (original), _h (high), _n (normal), _l (low)
            for suffix in quality_suffixes:
                if file_code_with_quality.endswith(suffix):
                    quality_defined = True
                    file_code = file_code_with_quality[: -len(suffix)]
                    quality_suffix = suffix # We might not need quality_suffix itself if form handles it
                    break
            ...
            if quality_defined:
                ...
                form_inputs = html.xpath('//form[@id="F1"]//input | //form[contains(@action, "/d/")]//input') # More generic form search
                if not form_inputs:
                    # Check for errors if form not found
                    error_div = html.xpath('//div[@class="alert alert-danger"][1]/text()')
                    if error_div and len(error_div) > 1: # Often error is second text node
                        raise ExtractionFailedException(f"Streamvid error (quality page): {error_div[1].strip()}")
                    raise ExtractionFailedException("Streamvid error: No form inputs found on quality download page.")
                ...
                for i in form_inputs:
                    ...
                    if name: # Ensure name exists
                        post_data[name] = value if value is not None else "" # Handle None values

                # POST to the same download_page_url
                async with await self._post(download_page_url, data=post_data) as post_response:
                ...
                script_elements = post_html.xpath('//script[contains(text(),"document.location.href")]/text()')
                if not script_elements:
                    error_div_post = post_html.xpath('//div[@class="alert alert-danger"][1]/text()')
                    if error_div_post and len(error_div_post) > 1:
                        raise ExtractionFailedException(f"Streamvid error (after POST): {error_div_post[1].strip()}")
                    raise ExtractionFailedException("Streamvid error: Direct link script not found after POST.")

                script_content = script_elements[0]
                direct_link_match = re.search(r'document\.location\.href\s*=\s*["\']([^"\']+)["\']', script_content)
                if not direct_link_match:
                    raise ExtractionFailedException("Streamvid error: Direct link not found in script pattern.")
                ...
                filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": download_page_url})
                return LinkResult(url=direct_link, filename=filename, size=size)

            else: # Quality not defined in URL, list available qualities
                qualities_urls = html.xpath('//div[@id="dl_versions"]//a/@href')
                qualities_texts = html.xpath('//div[@id="dl_versions"]//a/text()[normalize-space()]') # Get non-empty text nodes
                ...
                if qualities_urls and qualities_texts:
                    ...
                    cleaned_texts = [re.sub(r'\s+', ' ', text).strip() for text in qualities_texts if text.strip()]
                    ...
                    for i, q_url_path in enumerate(qualities_urls):
                        quality_name = cleaned_texts[i] if i < len(cleaned_texts) else q_url_path.split('_')[-1] if '_' in q_url_path else "Unknown"
                        # The q_url_path is relative, e.g. /d/xxxx_o. We need the suffix part.
                        suffix_match = re.search(r"(_[ohl])$", q_url_path.split('/')[-1]) # _o, _h, _l
                        if not suffix_match: # Try _n for normal or others
                             suffix_match = re.search(r"(_[a-zA-Z])$", q_url_path.split('/')[-1])

                        suffix_for_user = suffix_match.group(1) if suffix_match else "(see full path)"
                ...
                if error_not_found:
                    raise ExtractionFailedException(f"Streamvid error: {error_not_found[0].strip()}")
