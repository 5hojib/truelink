...
class Mp4UploadResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            page_url = urlunparse((parsed_url.scheme, parsed_url.netloc, normalized_path, '', '', ''))
            ...
            form1_inputs = html.xpath('//form[@name="F1"]//input | //form[button/@name="submit"]//input') # More specific
            if not form1_inputs : # Fallback to original broad search if specific not found
                 form1_inputs = html.xpath("//input")

            if not form1_inputs:
                # Check for errors like "File not found"
                if "File Not Found" in page_html_text or "File was removed" in page_html_text:
                    raise ExtractionFailedException("Mp4Upload error: File Not Found or removed.")
                raise ExtractionFailedException("Mp4Upload error: Could not find initial form inputs on page.")

            data1 = {i.get("name"): i.get("value", "") for i in form1_inputs if i.get("name")}
            if not data1: # If all inputs were nameless or no inputs
                 raise ExtractionFailedException("Mp4Upload error: No valid data extracted for first POST.")
            ...
            async with await self._post(page_url, data=data1, headers=headers_post1) as post1_response:
            ...
            form2_inputs = html_post1.xpath('//form[@name="F1"]//input | //form[button/@name="submit"]//input')
            if not form2_inputs:
                if "File Not Found" in post1_html_text or "download link generated" not in post1_html_text.lower(): # Check if error or unexpected page
                     raise ExtractionFailedException("Mp4Upload error: Second form inputs not found or page indicates an error after first POST.")
                # Sometimes the direct link might be here if structure changed
                direct_link_early = html_post1.xpath('//a[contains(@href,"mp4upload.com:183/d/")]/@href')
                if direct_link_early:
                    dl = direct_link_early[0]
                    filename, size = await self._fetch_file_details(dl, custom_headers={"Referer": page_url})
                    return LinkResult(url=dl, filename=filename, size=size)
                raise ExtractionFailedException("Mp4Upload error: Second form inputs not found after first POST.")


            data2 = {i.get("name"): (i.get("value") or "").replace(" ", "") for i in form2_inputs if i.get("name")}
            if not data2:
                 raise ExtractionFailedException("Mp4Upload error: No valid data extracted for second POST.")
            ...
            async with await self._post(page_url, data=data2, headers=headers_post1) as post2_response: # Re-use headers from post1
                direct_link = str(post2_response.url) # The URL after redirects

            if not direct_link or "mp4upload" not in urlparse(direct_link).netloc.lower():
                ...
                final_page_text = await post2_response.text() # Get text of the final page
                if "generating download link" in final_page_text.lower() or "timer" in final_page_text.lower():
                    raise ExtractionFailedException("Mp4Upload error: Stuck in a loop or timer page after second POST.")
                raise ExtractionFailedException(f"Mp4Upload error: Could not obtain final direct link URL after second POST. Ended at: {direct_link}")
