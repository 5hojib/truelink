    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            for i, segment in enumerate(path_segments):
                if (segment == 'v' or segment == 'e') and i + 1 < len(path_segments):
            ...
            if not _id:
                ...
                if cleaned_path_segments:
                    ...
                    raise InvalidURLException(f"Could not extract video ID from Streamtape URL: {url}")
            ...
            for script_text in scripts_content:
                if script_text and ('ideoooolink' in script_text or 'ideoolink' in script_text):
            ...
            if not target_script_content:
                ...
                raise ExtractionFailedException("Streamtape error: Required script content not found on page.")
            ...
            link_match = re.search(r"document\.getElementById\(['\"](?:ideoooolink|ideoolink)['\"]\)\.innerHTML\s*=\s*['\"]([^'\"]+)['\"]", target_script_content)
            ...
            if not link_match:
                ...
                full_link_pattern = r"['\"](/get_video\?id=" + re.escape(_id) + r"[^'\"]+)['\"]"
                match_get_video = re.search(full_link_pattern, target_script_content)

                if not match_get_video:
                    raise ExtractionFailedException("Streamtape error: Download link pattern not found in script.")
            ...
            filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": page_url})
