...
class ShrdskResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not short_id:
                raise InvalidURLException(f"Could not extract short ID from Shrdsk.me URL: {url}")
            ...
            async with await self._get(api1_url) as api_res1:
                ...
                try:
                    ...
                    raise ExtractionFailedException(f"Shrdsk API1 error: Failed to parse JSON. {e_json}. Response: {err_txt[:200]}")


            if "download_data" not in json_res1 or not json_res1["download_data"]:
                error_msg = json_res1.get("error", "Missing 'download_data' in API1 response or data is empty.")
            ...
            async with await self._get(download_page_url, allow_redirects=False) as download_res:
                # We expect a redirect (3xx status) with a 'Location' header
                if not (300 <= download_res.status < 400 and "Location" in download_res.headers):
                    # If not a redirect, something is wrong. Maybe the page shows an error.
                    page_text = await download_res.text()
                    if "File not found" in page_text or "link has expired" in page_text:
                         raise ExtractionFailedException("Shrdsk error: File not found or link expired on download page.")
                    raise ExtractionFailedException(
