...
class LinkBoxResolver(BaseResolver):
    ...
    async def _fetch_item_detail(self, item_id: str) -> None:
        ...
        try:
            async with await self._get(
                ...
                if response.status != 200:
                    ...
                    raise ExtractionFailedException(f"LinkBox API (detail) error {response.status}: {err_text[:200]}")
        ...
        if not item_info:
            raise ExtractionFailedException("LinkBox API (detail) error: itemInfo not found")

        filename = item_info.get("name", "unknown_file")
        sub_type = item_info.get("sub_type")
        if sub_type and isinstance(filename, str) and not filename.strip().endswith(f".{sub_type}"):
            filename += f".{sub_type}"

        if not self._folder_details.title: # Set folder title from the single item if not already set
        ...
        self._folder_details.contents.append(
            FileItem(filename=filename, url=item_url, size=size, path="") # Single item has no sub-path
        )
        if size:
    ...
    async def _fetch_list_recursive(self, share_token: str, parent_id: int = 0, current_path: str = ""):
        ...
        try:
            async with await self._get(
                ...
                if response.status != 200:
                    err_text = await response.text()
                    raise ExtractionFailedException(f"LinkBox API (list) error {response.status}: {err_text[:200]}")
                json_data = await response.json()
        except Exception as e:
            if isinstance(e, ExtractionFailedException): raise
        ...
        if not contents_list and parent_id == 0 and not self._folder_details.contents: # Empty if it's the root and no items yet
        ...
        for content_item in contents_list:
            ...
            if content_item.get("type") == "dir" and "url" not in content_item: # It's a sub-folder
                ...
                self._folder_details.contents.append(
                    FileItem(filename=filename, url=item_url, size=size, path=current_path)
                )
                if size:
                    self._folder_details.total_size += size
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        self._folder_details = FolderResult(title="", contents=[], total_size=0) # Reset for each call

        parsed_url = urlparse(url)
        path_segments = parsed_url.path.split('/')
        share_token = None
        if path_segments:
            share_token = path_segments[-1] # Assumes share token is the last part of the path
        ...
        params = {"shareToken": share_token, "pageSize": 1, "pid": 0} # pageSize 1 is enough to get shareType
        try:
            async with await self._get("https://www.linkbox.to/api/file/share_out_list", params=params) as response:
                if response.status != 200:
                    err_text = await response.text()
                    raise ExtractionFailedException(f"LinkBox API (initial check) error {response.status}: {err_text[:200]}")
                json_data = await response.json()
        except Exception as e:
            if isinstance(e, ExtractionFailedException): raise
        ...
        if initial_data.get("shareType") == "singleItem" and "itemId" in initial_data:
            ...
            if not self._folder_details.title and "dirName" in initial_data:
                 self._folder_details.title = initial_data["dirName"] or "LinkBox Content"
        ...
        if not self._folder_details.contents:
            ...
            if not self._folder_details.title:
                 raise ExtractionFailedException("LinkBox: No content found and no title obtained.")
