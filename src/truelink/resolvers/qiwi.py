    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not path_segments:
                raise InvalidURLException("Qiwi.gg error: Could not extract file ID from URL (empty path).")

            file_id = path_segments[-1] # Assume file ID is the last part of the path
            ...
            filename_elements = html.xpath('//h1[contains(@class,"TextHeading")]/text()') # Made class check more general
            ...
            if not filename_elements:
                ...
                if title_text and title_text[0].strip():
                     ...
                     potential_filename_from_title = title_text[0].split(" - ")[0].strip()
                     if '.' in potential_filename_from_title: # Check if it looks like a filename with extension
                          filename_elements = [potential_filename_from_title]

                if not filename_elements:
                    # Check for common error messages if filename element is not found
                    if "File not found" in page_html_text or "This file does not exist" in page_html_text:
                        raise ExtractionFailedException("Qiwi.gg error: File not found on page.")
                    raise ExtractionFailedException("Qiwi.gg error: Could not find filename element on page to determine extension.")


            full_filename = filename_elements[0].strip()
            if not full_filename or '.' not in full_filename:
                raise ExtractionFailedException(f"Qiwi.gg error: Extracted filename '{full_filename}' is invalid or missing extension.")
            ...
            if not file_extension: # Should not happen if previous check passed
                 raise ExtractionFailedException(f"Qiwi.gg error: Could not determine file extension from '{full_filename}'.")
            ...
            filename_from_details, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": url})

            # Use filename from page if Content-Disposition doesn't provide a better one.
            # full_filename already includes the extension.
            final_filename = filename_from_details if filename_from_details else full_filename
