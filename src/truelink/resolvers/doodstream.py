...
class DoodStreamResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            intermediate_link_elements = html.xpath("//div[contains(@class,'download-content')]//a/@href")
            if not intermediate_link_elements:
                # Fallback: check for other common patterns if 'download-content' changed
                intermediate_link_elements = html.xpath("//a[contains(@class,'btn-download') or contains(@class,'download_button')]/@href")
                if not intermediate_link_elements:
                    # Check if page content indicates an error (e.g., "File not found", "DMCA")
                    if "File not found" in page_html_text or "File has been removed" in page_html_text:
                        raise ExtractionFailedException("DoodStream error: File not found or removed.")
            ...
            if intermediate_path.startswith("//"):
                ...
                intermediate_url = f"{parsed_url.scheme}://{parsed_url.netloc}{intermediate_path}"
            ...
            async with await self._get(intermediate_url, headers=headers_intermediate) as intermediate_response:
            ...
            final_link_match = re.search(r"window\.open\s*\(\s*['\"]([^'\"]+)['\"]", intermediate_page_text)
            if not final_link_match:
                ...
                if "Error generating download link" in intermediate_page_text:
                     raise ExtractionFailedException("DoodStream error: Server reported an error generating the download link.")
                raise ExtractionFailedException(
            ...
            if not direct_link.startswith("http"):
                raise ExtractionFailedException(f"DoodStream error: Extracted final link is not absolute: {direct_link}")
            ...
            filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": fetch_referer})
