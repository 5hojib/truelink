...
PASSWORD_ERROR_MESSAGE_GOFILE = "GoFile link {} requires a password (append ::password to the URL)."
...
class GoFileResolver(BaseResolver):
    ...
    def __init__(self):
        ...
        self._account_token: str | None = None # To store the fetched token for API calls
    ...
    async def _get_gofile_token(self) -> str:
        ...
        api_url = "https://api.gofile.io/accounts" # Original used POST, let's verify.
        ...
        async with await self._post(api_url, data=None) as response: # POST with no body
            if response.status != 200:
                err_text = await response.text()
                raise ExtractionFailedException(f"GoFile: Failed to get token (status {response.status}). {err_text[:200]}")
            try:
                json_data = await response.json()
            except Exception as e_json:
                err_txt = await response.text()
                raise ExtractionFailedException(f"GoFile: Failed to parse token JSON. {e_json}. Response: {err_txt[:200]}")

        if json_data.get("status") != "ok" or "token" not in json_data.get("data", {}):
            raise ExtractionFailedException(f"GoFile: Failed to get valid token. API Response: {json_data.get('message', 'Unknown error')}")

        return json_data["data"]["token"]

    async def _fetch_gofile_links_recursive(self, content_id: str, password_hash: str, current_path: str = ""):
        ...
        if not self._account_token: # Should be fetched by resolve()
            raise ExtractionFailedException("GoFile: Account token not available for fetching links.")
                ...
                if response.status != 200:
                    try:
                        ...
                        if "error-passwordRequired" in status_msg:
                            ...
                            raise ExtractionFailedException(PASSWORD_ERROR_MESSAGE_GOFILE.format(f"ID: {content_id}"))
                        if "error-passwordWrong" in status_msg:
                            raise ExtractionFailedException("GoFile error: Incorrect password provided.")
                        if "error-notFound" in status_msg:
                            raise ExtractionFailedException(f"GoFile error: Content ID '{content_id}' not found.")
                        ...
                        raise ExtractionFailedException(f"GoFile API (contents) error {response.status}: {status_msg} - {err_detail[:200]}")
                    except Exception: # If error response is not JSON
                         err_text = await response.text()
                         raise ExtractionFailedException(f"GoFile API (contents) error {response.status}: {err_text[:200]}")
                json_data = await response.json()
        except Exception as e:
            if isinstance(e, ExtractionFailedException): raise
            raise ExtractionFailedException(f"GoFile API (contents) request for ID '{content_id}' failed: {e!s}") from e

        if json_data.get("status") != "ok":
             # This should ideally be caught by status code check above, but as a fallback.
            raise ExtractionFailedException(f"GoFile API (contents) returned non-ok status: {json_data.get('status', 'Unknown status')}")

        data_node = json_data.get("data")
        if not data_node:
            raise ExtractionFailedException("GoFile API (contents) error: 'data' node missing in response.")

        if not self._folder_details.title: # Set root folder title
            self._folder_details.title = data_node.get("name", content_id if data_node.get("type") == "folder" else "GoFile Content")

        children_nodes = data_node.get("children", {})
        for child_id, child_content in children_nodes.items(): # children is a dict of id: content_obj
            child_name = child_content.get("name", child_id)
            if child_content.get("type") == "folder":
                if not child_content.get("public", True): # Skip non-public subfolders unless behavior changes
                    continue
                new_path_segment = child_name
                full_new_path = os.path.join(current_path, new_path_segment) if current_path else new_path_segment
                await self._fetch_gofile_links_recursive(child_id, password_hash, full_new_path)
            else: # It's a file
                ...
                if "size" in child_content:
                    ...
                    if isinstance(size_val, (int, float)): # Gofile API usually provides size in bytes
                        size = int(size_val)

                self._folder_details.contents.append(
                    FileItem(filename=child_name, url=file_link, size=size, path=current_path)
                )
                if size:
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        self._folder_details = FolderResult(title="", contents=[], total_size=0) # Reset
        ...
        if not content_id:
            raise InvalidURLException("GoFile error: Could not extract content ID from URL.")

        # Hash password if provided
        password_hash = sha256(_password.encode("utf-8")).hexdigest() if _password else ""

        try:
            self._account_token = await self._get_gofile_token()
            await self._fetch_gofile_links_recursive(content_id, password_hash, "") # Start with empty path
            ...
            if "passwordRequired" in str(e) and not _password:
                raise ExtractionFailedException(PASSWORD_ERROR_MESSAGE_GOFILE.format(request_url)) from e
            raise e # Re-raise other extraction failures
        except Exception as e_outer:
            raise ExtractionFailedException(f"GoFile resolution failed: {e_outer!s}") from e_outer
        ...
        if not self._folder_details.contents:
            ...
            if not self._folder_details.title : # No title and no contents often means failure
                 raise ExtractionFailedException(f"GoFile: No downloadable content found for ID '{content_id}'. It might be empty, private, or password protected.")
