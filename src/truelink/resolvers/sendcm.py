...
def _speed_string_to_bytes_placeholder_sendcm(size_str: str) -> int:
    ...
    if "MB" in size_str: return int(num * (1024**2))
    if "KB" in size_str: return int(num * 1024)
    return int(num)

PASSWORD_ERROR_MESSAGE_SENDCM = "Send.cm link {} requires a password (append ::password to the URL)."
...
class SendCmResolver(BaseResolver):
    ...
    CF_BYPASS_API_URL = "https://cf.jmdkh.eu.org/v1" # Hardcoded from original script
    ...
    async def _cf_bypass_helper(self, url_to_bypass: str) -> str:
        ...
        try:
            async with await self._post(self.CF_BYPASS_API_URL, json=payload, headers=headers) as cf_response:
                if cf_response.status != 200:
                    err_text = await cf_response.text()
                    raise ExtractionFailedException(f"Send.cm (CF Bypass API) error {cf_response.status}: {err_text[:200]}")

                json_res = await cf_response.json()

            if json_res.get("status") == "ok" and "solution" in json_res and "response" in json_res["solution"]:
                return json_res["solution"]["response"] # This is expected to be HTML content
            else:
                error_detail = json_res.get("message", "CF Bypass failed or returned unexpected response.")
                raise ExtractionFailedException(f"Send.cm (CF Bypass API) error: {error_detail}. Full: {str(json_res)[:200]}")
        except Exception as e:
            if isinstance(e, ExtractionFailedException): raise
            raise ExtractionFailedException(f"Send.cm: Cloudflare bypass request failed for {url_to_bypass}. Error: {e!s}")


    async def _resolve_sendcm_file(self, page_url: str, file_id_override: str | None = None) -> LinkResult:
        ...
        if file_id_override is None: # Need to parse the page to get file_id
            try:
                ...
                if not file_id_elements:
                    raise ExtractionFailedException("Send.cm file error: file_id not found on page.")
                file_id_from_page = file_id_elements[0]
            except Exception as e: # Catch issues getting the page or parsing file_id
                 if isinstance(e, ExtractionFailedException): raise
                 # If GET fails (e.g. CF protection), this is where CF bypass might be needed.
                 # For now, re-raise.
                 raise ExtractionFailedException(f"Send.cm: Failed to get file page {request_url}. Error: {e!s}")
        ...
        if not actual_file_id: # Should not happen if logic is correct
            raise ExtractionFailedException("Send.cm file error: Could not determine file_id.")

        post_data = {"op": "download2", "id": actual_file_id}
        if _password and (password_needed_on_page or file_id_override): # Add password if provided and needed/likely needed
            post_data["password"] = _password

        # POST to get the direct link
        # allow_redirects=False to catch Location header
        async with await self._post("https://send.cm/", data=post_data, allow_redirects=False) as dl_response:
            if "Location" in dl_response.headers and 300 <= dl_response.status < 400:
                ...
                filename, size = await self._fetch_file_details(direct_link, custom_headers=fetch_headers)
                return LinkResult(url=direct_link, filename=filename, size=size)
            else: # No redirect or not a redirect status
                if password_needed_on_page and not _password: # Or if we tried without password and it failed
                    raise ExtractionFailedException(PASSWORD_ERROR_MESSAGE_SENDCM.format(request_url))
                # Other errors
                error_text_snippet = await dl_response.text()
                raise ExtractionFailedException(f"Send.cm file error: Could not get direct link. Status {dl_response.status}. Response: {error_text_snippet[:200]}")
    ...
    async def _get_file_link_for_folder(self, file_id: str) -> str | None:
        ...
        try:
            async with await self._post("https://send.cm/", data={"op": "download2", "id": file_id}, allow_redirects=False) as res:
    ...
    async def _process_sendcm_folder_page(self, html_page_content: str, current_path: str, folder_details_obj: FolderResult):
        ...
        for folder_elem in subfolder_elements:
            ...
            try:
                subfolder_html_content = await self._cf_bypass_helper(subfolder_full_url)
                new_path = os.path.join(current_path, folder_name)
                await self._process_sendcm_folder_page(subfolder_html_content, new_path, folder_details_obj)
            except Exception:
        ...
        for row in file_rows:
            ...
            if file_direct_link:
                ...
                folder_details_obj.contents.append(
                    FileItem(filename=file_name, url=file_direct_link, size=size_bytes, path=current_path)
                )
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        parsed_url = urlparse(url.split("::")[0]) # Use URL without password for path checks

        # Handle direct file links (e.g. /d/xxxx or /xxxx if no /s/ or /d/)
        if "/d/" in parsed_url.path or \
           ("/s/" not in parsed_url.path and parsed_url.path.count('/') <= 2 and parsed_url.path != "/"):
        ...
        if "/s/" in parsed_url.path:
            ...
            elif len(path_parts) >= 2 and path_parts[0] == 's': # /s/id (no name in URL)
            ...
            try:
                ...
                main_folder_html = await self._cf_bypass_helper(url.split("::")[0]) # Use URL without password
                await self._process_sendcm_folder_page(main_folder_html, folder_details.title, folder_details)
            except Exception as e:
                raise ExtractionFailedException(f"Send.cm: Failed to process folder '{url}'. Error: {e!s}")
            ...
            if not folder_details.contents:
                ...
                if not folder_details.title: # Should have a title by now
                    raise ExtractionFailedException(f"Send.cm: No files found in folder '{url}' and no title derived.")
            ...
            if len(folder_details.contents) == 1:
                ...
                if folder_details.title == single_item.filename and not single_item.path: # Path check ensures it's not in a sub-sub-folder
                     # Original returned (url, header). We use header for _fetch if needed, not in result.
                    return LinkResult(url=single_item.url, filename=single_item.filename, size=single_item.size)
