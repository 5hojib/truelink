    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not direct_link_elements:
                ...
                fallback_links = html.xpath("//a[contains(translate(@class, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'download') and @href]/@href")
                if not fallback_links:
                    # Check for error messages on page
                    error_messages = html.xpath("//div[contains(@class, 'alert-danger')]/text() | //div[contains(@class, 'error')]/text()")
                    if error_messages:
                        raise ExtractionFailedException(f"Upload.ee error: {error_messages[0].strip()}")
                    if "File not found" in response_text or "File has been deleted" in response_text:
                         raise ExtractionFailedException("Upload.ee error: File not found or has been deleted.")
                    raise ExtractionFailedException("Upload.ee error: Direct download link element (id='d_l' or fallback) not found.")
