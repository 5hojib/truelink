    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not path_segments:
                raise InvalidURLException("StreamHub: Invalid URL, no path segments found.")
            ...
            download_page_url = f"{parsed_url.scheme}://{parsed_url.netloc}/d/{file_code}"
            ...
            if not form_inputs:
                ...
                form_inputs = html.xpath('//form[contains(@action, "/d/")]//input | //form[button/@type="submit"]//input')
                if not form_inputs:
                    # Check for error messages if form not found
                    error_div = html.xpath('//div[contains(@class, "alert-danger")]/text()') # More generic error check
                    if error_div: # Get all text from error div
                        error_message = "".join(error_div).strip()
                        raise ExtractionFailedException(f"StreamHub error (page): {error_message}")
                    if "File not found" in page_html_text:
                         raise ExtractionFailedException("StreamHub error: File not found.")
                    raise ExtractionFailedException("StreamHub error: Download form inputs not found on page.")
            ...
            async with await self._post(download_page_url, data=post_data, headers=post_headers) as post_response:
            ...
            direct_link_elements = post_html.xpath('//a[contains(@class,"downloadbtn")]/@href')
            if not direct_link_elements:
                # Check for error messages after POST
                error_div_post = post_html.xpath('//div[contains(@class, "alert-danger")]/text()')
                if error_div_post:
                    error_message_post = "".join(error_div_post).strip()
                    raise ExtractionFailedException(f"StreamHub error (after POST): {error_message_post}")
                raise ExtractionFailedException("StreamHub error: Direct download button/link not found after POST.")

            direct_link = direct_link_elements[0]

            # Ensure link is absolute
            if not direct_link.startswith("http"):
                direct_link = urljoin(download_page_url, direct_link) # Use download_page_url as base
