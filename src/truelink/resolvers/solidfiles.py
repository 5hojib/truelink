...
class SolidFilesResolver(BaseResolver):
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            match = re.search(r"viewerOptions['\"]?\s*,\s*(\{.*?\})\s*\)\s*;", page_source, re.DOTALL | re.IGNORECASE)
            if not match:
                 ...
                 dl_links = html.xpath('//a[contains(translate(@href, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "solidfilesusercontent.com")]/@href')
                 if not dl_links:
                     dl_links = html.xpath('//a[contains(@class, "download-button") or contains(@id, "download-button")]/@href')
                 ...
                 if dl_links:
                     ...
                     filename_guess = filename_from_page_title[0].replace("SolidFiles - ", "").strip() if filename_from_page_title else None

                     filename, size = await self._fetch_file_details(direct_link)
                     return LinkResult(url=direct_link, filename=filename if filename else filename_guess, size=size)
            ...
            filename_from_options = options_data.get('nodeName') or options_data.get('name')

            filename, size = await self._fetch_file_details(direct_link)

            return LinkResult(url=direct_link, filename=filename if filename else filename_from_options, size=size)
