...
from truelink.types import FolderResult, LinkResult # FolderResult needed for type hint compliance
...
class HxFileResolver(BaseResolver):
    ...
    COOKIE_FILE = "hxfile.txt" # Assuming this file is in the current working directory
    ...
    def _load_cookies_from_file(self) -> dict[str, str] | None:
        ...
        if not os.path.isfile(self.COOKIE_FILE):
            ...
            raise ExtractionFailedException(f"HxFile error: Cookie file '{self.COOKIE_FILE}' not found.")
        ...
        try:
            ...
            raise ExtractionFailedException(f"HxFile error: Failed to load cookies from '{self.COOKIE_FILE}': {e!s}")

    async def resolve(self, url: str) -> LinkResult | FolderResult:
        """Resolve HxFile.co URL"""

        loaded_cookies = self._load_cookies_from_file() # This might raise if file not found

        try:
            # Normalize URL: remove .html if present
            if url.strip().endswith(".html"):
                normalized_url = url[:-5]
            else:
                normalized_url = url

            file_code_match = normalized_url.split("/")
            if not file_code_match:
                raise ExtractionFailedException("HxFile error: Could not extract file code from URL.")
            ...
            if not direct_link_elements:
                ...
                error_message = html.xpath("//div[contains(@class,'alert-danger')]/text()")
                if error_message:
                    raise ExtractionFailedException(f"HxFile error: {error_message[0].strip()}")
                # Check for login/premium messages
                if "This link requires a premium account" in response_text or "Login to download" in response_text:
                     raise ExtractionFailedException("HxFile error: Link may require premium account or login.")
                raise ExtractionFailedException("HxFile error: Direct download link not found on page.")
