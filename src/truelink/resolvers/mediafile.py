...
from lxml.html import fromstring # Keep lxml.html for consistency if used elsewhere, or use fromstring directly
    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not match:
                ...
                postvalue_direct = re.search(r"showFileInformation(.*);", response_text)
                if not postvalue_direct:
                    raise ExtractionFailedException("Unable to find initial download link or post value on the page.")

                # If found directly, the current URL is the one to be used as referer for AJAX
                download_url = str(response.url) # Get the final URL after any redirects
                ...
                async with await self._get(download_url, headers={"Referer": url}) as res_download_page:
                    download_page_text = await res_download_page.text()

                postvalue = re.search(r"showFileInformation(.*);", download_page_text)
                if not postvalue:
                    raise ExtractionFailedException("Unable to find post value on download page.")
                postid = postvalue.group(1).replace("(", "").replace(")", "")

            # AJAX POST request
            ajax_headers = {"X-Requested-With": "XMLHttpRequest", "Referer": download_url}
            async with await self._post(
                ...
                except Exception as json_error: # More specific error for JSON parsing
                    raise ExtractionFailedException(f"Failed to parse JSON response from file_details: {json_error}")


            if "html" not in json_response:
                raise ExtractionFailedException("AJAX response does not contain 'html' key.")
            ...
            potential_links = re.findall(r'https://[^\s"\']+', html_content_from_ajax)
            token_links = [link for link in potential_links if "download_token" in link]

            if len(token_links) < 2: # Or check if len(token_links) == 0 or 1 depending on expected behavior
                ...
                # if no token_links, try any link from the ajax response
                    ...
                    raise ExtractionFailedException("No suitable download link with 'download_token' found in AJAX response.")
            else:
                direct_link = token_links[1] # Original logic: take the second one

            filename, size = await self._fetch_file_details(direct_link, custom_headers={"Referer": download_url})
