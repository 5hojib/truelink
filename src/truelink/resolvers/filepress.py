    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not file_id_match or not file_id_match[-1]:
                raise InvalidURLException("FilePress error: Could not extract file ID from URL.")
            ...
            if not api_file_id:
                 raise ExtractionFailedException("FilePress error: Could not determine API file ID from filebee URL.")
            ...
            json_data1 = {"id": api_file_id, "method": "publicDownlaod"} # 'publicDownlaod' typo
            headers1 = {"Referer": f"{api_base_scheme}://{api_base_host}"}

            async with await self._post(api1_url, json=json_data1, headers=headers1) as api_res1:
                if api_res1.status != 200:
                    err_txt = await api_res1.text()
                    raise ExtractionFailedException(f"FilePress API1 error ({api_res1.status}): {err_txt[:200]}")
                try:
                    json_res1 = await api_res1.json()
                except Exception as e_json:
                    err_txt = await api_res1.text()
                    raise ExtractionFailedException(f"FilePress API1 error: Failed to parse JSON. {e_json}. Response: {err_txt[:200]}")


            if "data" not in json_res1 or not json_res1["data"]:
                status_text = json_res1.get("statusText", "Missing 'data' in API1 response or data is empty.")
                raise ExtractionFailedException(f"FilePress API1 error: {status_text}")
            ...
            json_data2 = {"id": intermediate_id, "method": "publicDownlaod"} # 'publicDownlaod' typo
            ...
            async with await self._post(api2_url, json=json_data2, headers=headers2) as api_res2:
                if api_res2.status != 200:
                    err_txt = await api_res2.text()
                    raise ExtractionFailedException(f"FilePress API2 error ({api_res2.status}): {err_txt[:200]}")
                try:
                    json_res2 = await api_res2.json()
                except Exception as e_json:
                    err_txt = await api_res2.text()
                    raise ExtractionFailedException(f"FilePress API2 error: Failed to parse JSON. {e_json}. Response: {err_txt[:200]}")

            if "data" not in json_res2 or not json_res2["data"]:
                status_text = json_res2.get("statusText", "Missing 'data' in API2 response or data is empty.")
                raise ExtractionFailedException(f"FilePress API2 error: {status_text}")

            gdrive_file_id = json_res2["data"]
            direct_link = f"https://drive.google.com/uc?id={gdrive_file_id}&export=download"
