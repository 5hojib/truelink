    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not form_elements:
                raise ExtractionFailedException("Unable to find form data for POST request")
            ...
            async with await self._post(url, data=data, headers=post_headers) as post_response:
                post_response_text = await post_response.text()

            html_post = fromstring(post_response_text)

            success_link_elements = html_post.xpath('//div[@class="alert alert-success mb-0"]//a')
            if not success_link_elements:
                # Attempt to find an error message if the success link isn't there
                error_elements = html_post.xpath('//div[contains(@class, "alert-danger")]/text()')
                if error_elements:
                    error_message = "".join(error_elements).strip()
                    raise ExtractionFailedException(f"UploadHaven error: {error_message}")
                raise ExtractionFailedException("Unable to find download link after POST")

            direct_link = success_link_elements[0].get("href")
            if not direct_link:
                raise ExtractionFailedException("Found link element but href is missing")
