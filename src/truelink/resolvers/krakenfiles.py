    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            if not post_url_elements:
                ...
                post_url_elements = html.xpath('//form[contains(@action, "/download/")]/@action')
                if not post_url_elements:
                    raise ExtractionFailedException("KrakenFiles error: Unable to find post form action URL.")
            ...
            if post_url_path.startswith("//"):
                ...
                post_url = f"{response.url.scheme}://{response.url.host}{post_url_path}"
            elif not post_url_path.startswith("http"):
                 post_url = urljoin(str(response.url), post_url_path) # Use response.url as base
            else:
            ...
            if not token_elements:
                ...
                token_elements = html.xpath('//form[@id="dl-form"]//input[@name="token"]/@value')
                if not token_elements:
                    raise ExtractionFailedException("KrakenFiles error: Unable to find token for POST request.")
            ...
            async with await self._post(post_url, data=post_data, headers=post_headers) as post_response:
                try:
                    ...
                    if "captcha" in text_response_snippet.lower():
                         raise ExtractionFailedException(f"KrakenFiles error: Captcha encountered. {text_response_snippet[:200]}")
                    raise ExtractionFailedException(f"KrakenFiles error: Failed to parse JSON response from POST. Error: {json_error}. Response: {text_response_snippet[:200]}")


            if json_response.get("status") != "ok":
                error_message = json_response.get("message", "POST request did not return 'ok' status.")
                if "url" not in json_response : # If status is not ok AND no url, it's likely an error
                    raise ExtractionFailedException(f"KrakenFiles error: {error_message}")
            ...
            if "url" not in json_response:
                raise ExtractionFailedException("KrakenFiles error: 'url' not found in JSON response after POST.")
