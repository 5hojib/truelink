    ...
    async def resolve(self, url: str) -> LinkResult | FolderResult:
        ...
        try:
            ...
            async with await self._get(url) as initial_response:
                canonical_url = str(initial_response.url) # This is the main transfer page URL
            ...
            if len(path_segments) < 2 or path_segments[0] != 'downloads':
                ...
                if not path_segments or path_segments[0] != 'downloads':
                     raise InvalidURLException(f"Invalid WeTransfer URL format. Expected '/downloads/...' path: {canonical_url}")

                if len(path_segments) >= 3: # e.g. /downloads/transferId/securityHash
                    transfer_id = path_segments[1]
                    security_hash = path_segments[2] # This might be recipient_id if path is longer
                    if len(path_segments) >= 4: # /downloads/transferId/recipientId/securityHash
                        security_hash = path_segments[3]
                elif len(path_segments) == 2 and path_segments[0] == 'downloads': # /downloads/transferId (security hash might be missing or implied)
                    ...
                    raise InvalidURLException(f"WeTransfer URL does not seem to contain a security hash: {canonical_url}")
                else:
                    raise InvalidURLException(f"Could not parse transfer_id and security_hash from WeTransfer URL: {canonical_url}")
            ...
            async with await self._post(api_url, json=json_data, headers=headers) as api_response:
                if api_response.status != 200:
                    error_text = await api_response.text()
                    try: # Try to parse JSON error from WeTransfer
                        json_err_data = await api_response.json(content_type=None) # Allow any content type for error
                ...
                try:
                    ...
                    raise ExtractionFailedException(f"WeTransfer API error: Failed to parse JSON. {e_json}. Response: {err_txt[:200]}")
            ...
            if "direct_link" in json_resp_data:
                ...
                filename = json_resp_data.get("display_name") or json_resp_data.get("name")
                size_api = json_resp_data.get("size") # Typically in bytes

                # Fetch details from the link to confirm/get more accurate info
                header_filename, header_size = await self._fetch_file_details(direct_link)
                ...
                return LinkResult(url=direct_link, filename=final_filename, size=final_size)

            # Handle other API responses (errors, etc.)
            if "message" in json_resp_data:
                raise ExtractionFailedException(f"WeTransfer API error: {json_resp_data['message']}")
            if "error" in json_resp_data:
                raise ExtractionFailedException(f"WeTransfer API error: {json_resp_data['error']}")

            raise ExtractionFailedException("WeTransfer error: 'direct_link' not found in API response and no specific error message.")
